...
export async function installAllTools(
	updateExistingToolsOnly: boolean = false,
) {
			...
			return res.then(
				(sofar) =>
					new Promise<string[]>((resolve, reject) => {
						...
						const modulesOffForTool =
							modulesOff ||
							disableModulesForWildcard(tool, goVersion);
						...
						if (path.isAbsolute(toolBinPath) && isGocode(tool)) {
							closeToolPromise = new Promise<boolean>(
								(innerResolve) => {
									cp.execFile(
										toolBinPath,
										["close"],
						...
						closeToolPromise.then((success) => {
							...
							if (modulesOffForTool) {
								...
								importPath = getImportPathWithVersion(
									tool,
									tool.version,
									goVersion,
								);
			...
			switch (selected) {
				...
				case "Release Notes":
					vscode.commands.executeCommand(
						"vscode.open",
						vscode.Uri.parse(
							"https://github.com/golang/go/issues/33030#issuecomment-510151934",
						),
					);
...
export function updateGoPathGoRootFromConfig(): Promise<void> {
	...
	if (
		...
		(<string>process.env[pathEnvVar])
			.split(path.delimiter)
			.indexOf(goRuntimeBasePath) === -1
	) {
		// Place the goRuntimeBasePath to the front so tools use the same version of go.
		process.env[pathEnvVar] =
			goRuntimeBasePath + path.delimiter + process.env[pathEnvVar];
	}

	return new Promise<void>((resolve, reject) => {
		cp.execFile(
			goRuntimePath,
			["env", "GOPATH", "GOROOT", "GOPROXY"],
			(err, stdout, stderr) => {
				if (err) {
					return reject();
				}
...
export async function offerToInstallTools() {
	...
	const usingSourceGraph =
		getToolFromToolPath(getLanguageServerToolPath()) === "go-langserver";
		...
		vscode.window
			...
			.then((selected) => {
				if (selected === installLabel) {
					...
					const inspectLanguageServerSetting =
					...
					if (inspectLanguageServerSetting.globalValue === true) {
						...
						inspectLanguageServerSetting.workspaceFolderValue ===
						true
					) {
						goConfig.update(
							"useLanguageServer",
