...
function showPackageFiles(
	pkg: string,
	showAllPkgsIfPkgNotFound: boolean,
	workDir: string,
) {
	...
	cp.execFile(
		...
		[
			"list",
			"-f",
			"{{.Dir}}:{{.GoFiles}}:{{.TestGoFiles}}:{{.XTestGoFiles}}",
			pkg,
		],
		options,
		(err, stdout, stderr) => {
			...
			if (matches) {
				...
				vscode.window
					...
					.then((file) => {
						...
						vscode.workspace
							.openTextDocument(path.join(dir, file))
							.then((document) => {
