...
export class GoRunTestCodeLensProvider extends GoBaseCodeLensProvider {
		...
		if (
			symbols[0].children.some(
				(sym) =>
					...
					this.benchmarkRegex.test(sym.name),
			)
		) {
	...
	private async getCodeLensForFunctions(
		...
		const testPromise = getTestFunctions(document, token).then(
			(testFunctions) => {
		...
		const benchmarkPromise = getBenchmarkFunctions(document, token).then(
			(benchmarkFunctions) => {
