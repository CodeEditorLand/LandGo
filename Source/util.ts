...
import {
	extensionId,
	sendTelemetryEventForGoVersion,
	sendTelemetryEventForKillingProcess,
} from "./telemetry";
...
export class GoVersion {
	...
	constructor(version: string) {
		...
		const matchesDevel = /go version devel \+(.[a-zA-Z0-9]+).*/.exec(
			version,
		);
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (line.match(/^(\s)*(\/\*.*\*\/)*\s*\)/)) {
			...
			if (ret.imports[ret.imports.length - 1].end === -1) {
				...
				if (importPkgMatch) {
					ret.imports[ret.imports.length - 1].pkgs.push(
						importPkgMatch[1],
					);
				}
			}
...
export function canonicalizeGOPATHPrefix(filename: string): string {
	...
	for (const workspace of workspaces) {
		...
		if (
			filenameLowercase.substring(0, workspace.length) ===
	...
	return new Promise<GoVersion>((resolve) => {
		cp.execFile(goRuntimePath, ["version"], {}, (err, stdout, stderr) => {
			...
			if (!cachedGoVersion.sv && !cachedGoVersion.isDevel) {
				if (err || stderr) {
					console.log(
						`Error when running the command "${goRuntimePath} version": `,
						err || stderr,
					);
				} else {
					console.log(
	...
	switch (goVersion.sv.major) {
		...
		case 1:
			vendorSupport =
				...
				((goVersion.sv.minor === 5 || goVersion.sv.minor === 6) &&
...
export function isPositionInString(
	...
	const escapedDoubleQuotesCnt = (
		lineTillCurrentPosition.match(/\\\"/g) || []
	).length;
...
export function getCurrentGoPath(workspaceUri?: vscode.Uri): string {
	...
	const currentFilePath = fixDriveCasingInWindows(
		activeEditorUri && activeEditorUri.fsPath,
	);
	...
	if (config["inferGopath"] === true) {
		inferredGopath =
			getInferredGopath(currentRoot) ||
			getInferredGopath(currentFilePath);
...
export function getExtensionCommands(): any[] {
	...
	const extensionCommands: any[] = vscode.extensions
		...
		.packageJSON.contributes.commands.filter(
			(x: any) => x.command !== "go.show.commands",
		);
...
export function getImportPath(text: string): string {
	...
	const singleLineImportMatches = text.match(
	...
	const groupImportMatches = text.match(
		/^\s*([a-z,A-Z,_,\.]\w*\s+)?\"([^\"]+)\"/,
	);
...
export function guessPackageNameFromFile(filePath: string): Promise<string[]> {
	return new Promise((resolve, reject) => {
		...
		if (
			segments.length === 0 ||
			!/[a-zA-Z_]\w*/.test(segments[segments.length - 1])
		) {
			return reject();
		}
		...
		if (diagnosticCollection === buildDiagnosticCollection) {
			...
			if (lintDiagnosticCollection.has(fileUri)) {
				lintDiagnosticCollection.set(
					...
					deDupeDiagnostics(
						newDiagnostics,
						lintDiagnosticCollection.get(fileUri).slice(),
					),
				);
			}
			...
			if (vetDiagnosticCollection.has(fileUri)) {
				vetDiagnosticCollection.set(
					...
					deDupeDiagnostics(
						newDiagnostics,
						vetDiagnosticCollection.get(fileUri).slice(),
					),
				);
			}
			...
			newDiagnostics = deDupeDiagnostics(
				buildDiagnosticCollection.get(fileUri).slice(),
				newDiagnostics,
			);
		}
...
function deDupeDiagnostics(
	...
	return otherDiagnostics.filter(
		(x) => buildDiagnosticsLines.indexOf(x.range.start.line) === -1,
	);
}
...
export function makeMemoizedByteOffsetConverter(
	...
	return (byteOffset: number) => {
		...
		if (byteDelta > 0) {
			...
			charDelta = -buffer.toString("utf8", byteOffset, nearest.key)
				.length;
		}
 ...
 * @param position
 */
export function isPositionInComment(
	document: vscode.TextDocument,
	position: vscode.Position,
): boolean {
	const lineText = document.lineAt(position.line).text;
