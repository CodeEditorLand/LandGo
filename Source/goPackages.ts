	...
	return new Promise<Map<string, PackageInfo>>((resolve, reject) => {
		...
		cmd.on("close", () => {
			...
			if (errorMsg) {
				if (
					errorMsg.startsWith(
						"flag provided but not defined: -workDir",
					)
				) {
 ...
 * @returns Map<string, string> mapping between package import path and package name
 */
export function getImportablePackages(
	filePath: string,
	...
	return Promise.all([isVendorSupported(), getAllPackagesPromise]).then(
		([vendorSupported, pkgs]) => {
			...
			const currentWorkspace = getCurrentGoWorkspaceFromGOPATH(
				getCurrentGoPath(),
				fileDirPath,
			);
			...
			pkgs.forEach((info, pkgPath) => {
				...
				const relativePkgPath = getRelativePackagePath(
					fileDirPath,
					currentWorkspace,
					pkgPath,
				);
				...
				const allowToImport = isAllowToImportPackage(
					fileDirPath,
					currentWorkspace,
					relativePkgPath,
				);
...
function getRelativePackagePath(
	...
	if (vendorIndex > -1) {
		const rootProjectForVendorPkg = path.join(
			currentWorkspace,
		...
		if (
			relativePathForVendorPkg &&
			currentFileDirPath.startsWith(rootProjectForVendorPkg) &&
			!subVendor
		) {
	...
	return new Promise<Map<string, string>>((resolve, reject) => {
		const childProcess = cp.spawn(
			...
			[
				"list",
				"-f",
				"ImportPath: {{.ImportPath}} FolderPath: {{.Dir}}",
				"./...",
			],
...
function isAllowToImportPackage(
	toDirPath: string,
	currentWorkspace: string,
	pkgPath: string,
) {
