...
function vscodeKindFromGoCodeClass(
	kind: string,
	type: string,
...
export class GoCompletionItemProvider
	implements vscode.CompletionItemProvider, vscode.Disposable
{
	...
	public async provideCompletionItemsInternal(
		...
		const pkgStatementCompletions =
			await getPackageStatementCompletions(document);
		...
		return this.ensureGoCodeConfigured(document.uri, config).then(() => {
			return new Promise<vscode.CompletionItem[] | vscode.CompletionList>(
				(resolve, reject) => {
					const filename = document.fileName;

					const lineText = document.lineAt(position.line).text;

					const lineTillCurrentPosition = lineText.substr(
						0,
						position.character,
					);

					const autocompleteUnimportedPackages =
						config["autocompleteUnimportedPackages"] === true &&
						!lineText.match(/^(\s)*(import|package)(\s)+/);

					// triggering completions in comments on exported members
					const commentCompletion = getCommentCompletion(
						document,
						position,
					);
					...
					return this.runGoCode(
						...
						if (
							...
							const pkgPath = this.getPackagePathFromLine(
							...
							if (pkgPath.length === 1) {
								...
								const posToAddImport = document.offsetAt(
								...
								inputText =
									inputText.substr(0, posToAddImport) +
									textToAdd +
									inputText.substr(posToAddImport);
								...
								return this.runGoCode(
									...
									newsuggestions.forEach((item) => {
										item.additionalTextEdits =
											getTextEditForAddImport(pkgPath[0]);
									});
	...
	private runGoCode(
		...
		return new Promise<vscode.CompletionItem[]>((resolve, reject) => {
			...
			if (this.isGoMod) {
				const unimportedPkgIndex = this.gocodeFlags.indexOf(
					"-unimported-packages",
				);
			...
			const p = cp.spawn(
				gocode,
				[...this.gocodeFlags, "autocomplete", filename, "" + offset],
				{ env },
			);
			...
			p.on("close", (code) => {
				try {
					if (code !== 0) {
						if (
							stderr.indexOf(
								`rpc: can't find service Server.AutoComplete`,
							) > -1 &&
							!this.killMsgShown
						) {
					...
					const results = <[number, GoCodeSuggestion[]]>(
					...
					if (results && results[1]) {
						for (const suggest of results[1]) {
							...
							const item = new ExtendedCompletionItem(
								suggest.name,
							);

							item.kind = vscodeKindFromGoCodeClass(
								suggest.class,
								suggest.type,
							);
							...
							if (
								!areCompletionsForPackageSymbols &&
								item.package &&
								item.package !== "builtin"
							) {
								areCompletionsForPackageSymbols = true;
							}

							if (suggest.class === "package") {
								const possiblePackageImportPaths =
									this.getPackageImportPath(item.label);
							...
							if (
								...
								((suggest.class === "func" &&
									lineText.substr(position.character, 2) !==
										"()") || // Avoids met() -> method()()
									(suggest.class === "var" &&
										...
										lineText.substr(
											position.character,
											1,
										) !== ")" && // Avoids snippets when typing params in a func call
										lineText.substr(
											position.character,
											1,
										) !== ",")) // Avoids snippets when typing params in a func call
							...
							if (
								...
								auxItem.insertText = new vscode.SnippetString(
									snippet,
								);
	...
	private ensureGoCodeConfigured(
		...
		if (
			...
			checkModSupport = isModSupported(fileuri).then(
				(result) => (this.isGoMod = result),
			);
		}

		const setPkgsList = getImportablePackages(currentFile, true).then(
			(pkgMap) => {
			...
			cp.execFile(gocode, ["set"], { env }, (err, stdout, stderr) => {
				...
				if (
					existingOptions.indexOf(
						`autobuild ${goConfig["gocodeAutoBuild"]}`,
					) === -1
				) {
					optionsToSet.push([
						"autobuild",
						goConfig["gocodeAutoBuild"],
					]);
				}

				if (
					existingOptions.indexOf(
						`package-lookup-mode ${goConfig["gocodePackageLookupMode"]}`,
					) === -1
				) {
					optionsToSet.push([
						"package-lookup-mode",
						goConfig["gocodePackageLookupMode"],
					]);
				}
...
function getCommentCompletion(
	...
	if (
		...
		if (memberType && memberType.length === 4) {
			suggestionItem = new vscode.CompletionItem(
				memberType[3],
				vscodeKindFromGoCodeClass(memberType[1], ""),
			);
		}
...
function getPackageCompletions(
	...
	const goWorkSpace = getCurrentGoWorkspaceFromGOPATH(
		getCurrentGoPath(),
		cwd,
	);
	...
	const currentPkgRootPath = (
		workSpaceFolder ? workSpaceFolder.uri.path : cwd
	).slice(goWorkSpace.length + 1);
	...
	allPkgMap.forEach((info: PackageInfo, pkgPath: string) => {
		...
		if (
			pkgName.startsWith(currentWord) &&
			importedPackages.indexOf(pkgName) === -1
		) {
			const item = new vscode.CompletionItem(
				pkgName,
				vscode.CompletionItemKind.Keyword,
			...
			item.sortText = isStandardPackage
				? "za"
				: pkgPath.startsWith(currentPkgRootPath)
					? "zb"
					: "zc";
...
async function getPackageStatementCompletions(
	document: vscode.TextDocument,
...
export async function getCompletionsWithoutGoCode(
	...
	const packageCompletions = getPackageCompletions(
		document,
		currentWord,
		pkgMap,
	);
